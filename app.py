...
async def get_ai_recommendations(username):
    ...
    candidate_films = await fetch_candidate_films(highly_rated_films)
    if not candidate_films: return []

    return score_and_rank_candidates(user_taste_vector, candidate_films, films_to_process)
...
async def fetch_user_film_data(username):
    ...
    for title, id, slug, rating in films_watched_data:
        films_to_process.append({
            "Title": title, "Slug": slug, "Rating": rating,
...
async def fetch_tmdb_details(film_dict):
    try:
        ...
        if search_results:
            details = await tmdb_client.movie(search_results[0].id).details()
            film_dict.update({
...
async def fetch_candidate_films(highly_rated_films):
    ...
    top_genres = [name for name, count in genre_counts.most_common(3)]
    ...
    print(f"Fetched {len(candidates)} unique candidate movies.")
    return list(candidates.values())

def score_and_rank_candidates(user_taste_vector, candidate_films, films_to_process):
    print("\nScoring and ranking candidates...")
    valid_candidates = [c for c in candidate_films if c.overview]
    ...
    for candidate, score in zip(valid_candidates, similarity_scores):
    ...
    return sorted(ai_recommendations, key=lambda x: x['score'], reverse=True)[:10]
